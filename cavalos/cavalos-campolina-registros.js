...
const { createPropriedadeCavaloRecord, updateStatusColeta, updateStatusBase } = require('../services/cavalos-services');
...
const SIGLA = 'CAVALOS-CAMPOLINA-REGISTROS'
// exports.sigla = SIGLA
let logUltimaLetraPesquisada = ''
let registrosColetados = 0
// Parametros
...
exports.runRobo = async (page, browser) => {
    ...
    try {
        ...
        await updateStatusColeta(SIGLA, false, new Date(), null, true, registrosColetados)

        // atualiza status base
        await updateStatusBase(SIGLA)

        console.log('Coleta finalizada para o cavalo')
        ...
        // atualiza status da coleta no banco
        await updateStatusColeta(SIGLA, true, new Date(), error.message, false, registrosColetados)
        throw error
    } finally {
        if (clientRedis != null) {
    ...
    for (idx; idx < 1; idx++) {
        ...
        for (idxCavalo; idxCavalo < arrayRegistros.urls.length; idxCavalo++) {
            ...
            if (Object.keys(registro).length > 0) {
                ...
                const coletado = await createPropriedadeCavaloRecord(registro, SIGLA)
                // TODO como resolver para passar a qtd de cavalos??
                if (coletado) registrosColetados++
